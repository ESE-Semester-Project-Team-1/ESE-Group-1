...
void requestPass(){
    ...
    mu.lock();
    message_transmitter_queue.push(msg1);
    mu.unlock();

}

void passPlatoon(){
    std::cout << "Passing platoon...\n";
    
    //delay
    std::this_thread::sleep_for(std::chrono::seconds(5));

    std::cout << "Platoon passed\n";

    message msg1 = message("Platoon passed", 0, 100, FOLLOWER, 1);
...
int main() {
    ...
    std::thread t2(receiverModule, sock);
    std::thread t3(behaviourModule);
